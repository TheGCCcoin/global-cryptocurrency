        ...
        for (std::map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->tx->vout.size(); i++) {
                ...
                if(nCoinType == ONLY_DENOMINATED) {
                    ...
                    // m.7 masternode coins [

                    found = pcoin->tx->vout[i].nValue == MN_COIN;//1000*COIN;

                    // l.23 log nCoinType == ONLY_1000 [
                    LogPrintf("nCoinType == ONLY_1000 pcoin->tx->vout[i].nValue = %i %s\n", pcoin->tx->vout[i].nValue, pcoin->tx->GetHash().GetHex().c_str());
                    // l.23 log nCoinType == ONLY_1000 ]
...
bool CWallet::SelectCoinsByDenominations(int nDenom, CAmount nValueMin, CAmount nValueMax, std::vector<CTxDSIn>& vecTxDSInRet, std::vector<COutput>& vCoinsRet, CAmount& nValueRet, int nPrivateSendRoundsMin, int nPrivateSendRoundsMax)
{
    ...
    // ( bit on if present )
    // bit 0 - 100TheGCCcoin+1
    // bit 1 - 10TheGCCcoin+1
    // bit 2 - 1TheGCCcoin+1
    // bit 3 - .1TheGCCcoin+1
...
//                if(fMasternodeMode && wtx.tx->vout[i].nValue == 1000*COIN) continue;
                if(fMasternodeMode && wtx.tx->vout[i].nValue == MN_COIN) continue;
...
//        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == 1000*COIN) continue; //masternode input
        if(fMasternodeMode && out.tx->tx->vout[out.i].nValue == MN_COIN) continue; //masternode input
        ...
        LOCK2(cs_main, cs_wallet);
        {
            ...
            while (true)
            {
                ...
                if (fUseInstantSend && nValueIn > sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE)*COIN) {
                    strFailReason += " " + strprintf(_("InstantSend doesn't support sending values that high yet. Transactions are currently limited to %1 TheGCCcoin."), sporkManager.GetSporkValue(SPORK_5_INSTANTSEND_MAX_VALUE));
                    return false;
                }
                ...
                if (nChange > 0)
                {
                    ...
                    if (nCoinType == ONLY_DENOMINATED) {
                        ...
                        // change transaction isn't always pay-to-TheGCCcoin-address
                        CScript scriptChange;
