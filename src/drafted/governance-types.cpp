...
GovernanceObjectType GovernanceStringToType(std::string strType) {
    ...
    if(strType == "GCCNetwork") type = GCCNetwork;
    if(strType == "GCCNetworkVariable") type = GCCNetworkVariable;
...
std::string GovernanceTypeToString(GovernanceObjectType type) {
    ...
    if(type == GCCNetwork) s = "GCCNetwork";
    if(type == GCCNetworkVariable) s = "GCCNetworkVariable";
