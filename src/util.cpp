...
const char * const BITCOIN_CONF_FILENAME = "TheGCCcoin.conf";
const char * const BITCOIN_PID_FILENAME = "TheGCCcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "TheGCCcoin" is a composite category enabling all GCC-related debug output
                if(ptrCategory->count(std::string("TheGCCcoin"))) {
...
#else
    const char* pszModule = "TheGCCcoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GCCCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GCCCore
    // Mac: ~/Library/Application Support/GCCCore
    // Unix: ~/.TheGCCcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GCCCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GCCCore";
#else
    // Unix
    return pathRet / ".TheGCCcoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty TheGCCcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override TheGCCcoin.conf
